...
/**
 * Оставляю пока этот класс для истории, потом можно будет удалить
 * @author larin
 *
 */
public class GwtRpcSamplerGui extends HttpTestSampleGui {
    private static final long serialVersionUID = -6917992030933433477L;
    private static final Logger log = LoggingManager.getLoggerForClass();
    private String requestJson;
    private String responceJson;

    public GwtRpcSamplerGui() {
        super();
        init();
    }

    // For use by AJP
    protected GwtRpcSamplerGui(boolean ajp) {
        super(ajp);
        init();
    }
    ...
    private void showJsonViewer(String json) {
        JsonViewer jsonViewer = new JsonViewer(getWindowForComponent(GwtRpcSamplerGui.this), json);
