...
import static ru.intertrust.cm.core.gui.impl.client.util.BusinessUniverseConstants.CHOSE_DATE_RANGE_LABEL;
...
public class FormRangeDatePicker extends RangeDatePicker {
    ...
    protected void initWidget(Date startDate, Date endDate, boolean showTime, boolean showSeconds) {
        ...
        Panel forTodayPanel = initDateSelector(LocalizeUtil.get(FOR_TODAY_LABEL), new FormRangeEnumDateClickHandler(LocalizeUtil.get(FOR_TODAY_LABEL)));
        container.add(forTodayPanel);

        Panel forYesterdayPanel = initDateSelector(LocalizeUtil.get(FOR_YESTERDAY_LABEL), new FormRangeEnumDateClickHandler(LocalizeUtil.get(FOR_YESTERDAY_LABEL)));
        container.add(forYesterdayPanel);

        Panel forLastWeekPanel = initDateSelector(LocalizeUtil.get(FOR_LAST_WEEK_LABEL), new FormRangeEnumDateClickHandler(LocalizeUtil.get(FOR_LAST_WEEK_LABEL)));
        container.add(forLastWeekPanel);

        Panel forLastYearPanel = initDateSelector(LocalizeUtil.get(FOR_LAST_YEAR_LABEL), new FormRangeEnumDateClickHandler(LocalizeUtil.get(FOR_LAST_YEAR_LABEL)));
        container.add(forLastYearPanel);

        Panel dateRangePanel = initDateSelectorWithPicker(LocalizeUtil.get(CHOSE_DATE_RANGE_LABEL));
