...
public class FormPanel extends WidgetsContainer implements IsWidget {
  ...
  public FormPanel(final FormDisplayData formDisplayData, FormPluginState state, EventBus eventBus) {
    this.formDisplayData = formDisplayData;
    this.state = state;
    this.eventBus = eventBus;
    panel = new FlowPanel();
    build();
    Application.getInstance().getEventBus().fireEvent(new WidgetBroadcastEvent(this,true));
  }

  //plugin is needed for initialization, setting after build method is late for main use case
  public FormPanel(final FormDisplayData formDisplayData, FormPluginState state, EventBus eventBus, Plugin plugin) {
    this.formDisplayData = formDisplayData;
    this.state = state;
    this.eventBus = eventBus;
    this.plugin = plugin;
