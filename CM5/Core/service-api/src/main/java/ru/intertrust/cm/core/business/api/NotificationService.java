...
public interface NotificationService {
     ...
     *            Тип уведомления
     * @param sender
     *            Идентификатор персоны отправителя, может быть nullБ в этом случае подставится текущая персона
     * @param addresseeList
     *            список адресатов. Может быть персона, группа, динамическая группа или контекстная роль
     * @param priority
     *            приоритет сообщения. Влияет на отображение данного сообщения, но не влияет на очередность отправки
     * @param context
     *            Контекст сообщения. Содержит информацию о объекте системы, относительно которой производится отправка
     *            уведомления
     */
    ...
    /**
     * Метод асинхронной отправки уведомления. Используется из метода sendOnTransactionSuccess. 
